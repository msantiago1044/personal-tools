...
export const BudgetGaugeCard: React.FC<BudgetGaugeCardProps> = ({ year, month }) => {
  ...
  const fetchBudgetComparison = async () => {
    setLoading(true);
    try {
      // Llamada directa a la función RPC en Supabase
  ...
  return (
    <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 text-slate-100">
      <div className="flex justify-between items-center mb-6">
        <div>
          <h3 className="text-lg font-bold text-white">Presupuesto del Mes vs Real</h3>
      ...
      <div className="space-y-5">
        {data.map((item) => {
          ...
          // Color del indicador linear según el margen de ahorro
          ...
          return (
            <div key={item.category_id} className="p-4 bg-slate-950/60 rounded-2xl border border-slate-800/80">
              <div className="flex justify-between items-center text-sm mb-2">
                <div className="flex items-center gap-2">
                  ...
                  />
                  <span className="font-semibold text-white">{item.category_name}</span>
                </div>

                <div className="text-right">
                  <span className="font-bold text-slate-200">${item.actual_amount.toFixed(2)}</span>
                  <span className="text-slate-500 text-xs"> / ${item.estimated_amount.toFixed(2)}</span>
                </div>
              </div>
              ...
              {/* Métricas inferiores: % gastado y gasto del año pasado */}
              <div className="flex justify-between items-center mt-2.5 text-xs text-slate-400">
                ...
                <span className="text-slate-500">
                  Año anterior ({year - 1}):{' '}
                  <strong className="text-slate-400">${item.spent_previous_year.toFixed(2)}</strong>
                </span>
              </div>
            </div>
          );
